...
// displays the add source modal dialog
util.promptSources = function(makeLinkMarkdown){
		...
		var close = function(){			
			...
			makeLinkMarkdown(author,date,title,sponsor,accessed,text);			
			}			
			return true;
		};
		// bind function to close event
		$("#dialog-form").bind("dialogbeforeclose",close);
		$("#dialog-form").bind("dialogclose",reset);
	...
	wmd.editor = function(previewRefreshCallback){
		...
		var makeSpritedButtonRow = function(){
			...
			function addSpacer() {
				...
				// reference button only
				var sourceButton = addButton("wmd-source-button", "Source", function(chunk, postProcessing, useDefaultText){
					return command.doSource(chunk, postProcessing, true);
				});
	...
	command.doSource = function(chunk, postProcessing, isImage){
		...
		if (chunk.endTag.length > 1) {
			...
			// The function to be executed when you enter a link and press OK or Cancel.
			// Marks up the link and adds the ref.
			var makeLinkMarkdown = function(author,date,title,sponsor,accessed,link){
			
				// form source string
				var startTagText = author + (date ? " " + date : "") + (author || date ? ", " : "");
				var endTagText = (sponsor || accessed ? ", " : "") + sponsor + (accessed ? ", accessed " + accessed : "");
				...
				if (link !== null) {				
					...
					var num = command.addLinkDef(chunk, linkDef);
					chunk.startTag = startTagText + "[";
					chunk.endTag = "][" + num + "]" + endTagText;
			...
			util.promptSources(makeLinkMarkdown);
