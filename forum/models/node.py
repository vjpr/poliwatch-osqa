...
class NodeContent(models.Model):
    ...
    # sources for promise and update
    #sources    = PickledObjectField()
    sources = models.TextField()

    date_promised = models.DateField(blank=True, null=True)
    #maker = models.ForeignKey(Politician, blank=True, null=True) #TODO Might need to remove null
    ...
    #added 
    @property
    def htmlSources(self):
        return self._as_markdown(self.sources)
    
    @property
    def datePromised(self):
        return self.date_promised;
    ...
    def tagname_list(self):
        if self.tagnames:
            tag = [name.strip() for name in self.tagnames.split(u' ') if name]
...
class Node(BaseModel, NodeContent):
    ...
    maker = models.ManyToManyField(Politician, through='PromiseMaker')
    ...
    def activate_revision(self, user, revision):
        ...
        self.date_promised = revision.date_promised
        self.status = revision.status
        #self.maker = revision.maker
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            ...
            self.active_revision = self._create_revision(self.author, 1, title=self.title, tagnames=self.tagnames,
                                                         body=self.body, sources=self.sources, status=self.status,
                                                         date_promised=self.date_promised)
