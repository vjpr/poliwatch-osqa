...
from forum.models.politician import PromiseMaker
...
class NodeEditAction(ActionProxy):
    def create_revision_data(self, initial=False, **data):
        ...
        if data.get('sources', None):
            revision_data['sources'] = data['sources']
        
        if data.get('status', None):
            revision_data['status'] = data['status']
            
        if data.get('date_promised', None):
            revision_data['date_promised'] = data['date_promised']
        
#        if data.get('maker', None):
#            revision_data['maker'] = data['maker']
...
class ReviseAction(NodeEditAction):
    ...
    def process_data(self, **data):
        ...
        # Add makers
        for maker in data['maker']:
            if not (PromiseMaker.objects.filter(maker=maker,promise=self.node).exists()):
                temp = PromiseMaker(maker=maker,promise=self.node)
                temp.save()
        # Delete removed makers
        for pm in PromiseMaker.objects.filter(promise=self.node):
            if not (pm.maker in makers):
                PromiseMaker.delete(pm)
