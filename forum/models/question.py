...
class Question(Node):
    ...
    def current_status(self):
        if (self.accepted_answers.count() > 0): 
            return self.accepted_answers.order_by('-date_promised')[0].get_status_display
    ...
    def currentStatusId(self):
        if (self.accepted_answers.count() > 0): 
            return self.accepted_answers.order_by('-date_promised')[0].status
    
        return "not_yet_rated"
    
    @property
    def current_status_headline(self):
        if (self.accepted_answers.count() > 0): 
            return self.accepted_answers.order_by('-date_promised')[0].body
