...
class QuestionViewBadge(AbstractBadge):
    ...
    def description(self):
        return _('Added a promise with %s views') % str(self.nviews)
...
class QuestionScoreBadge(NodeScoreBadge):
    ...
    @property
    def description(self):
        return _('Promise voted up %s times') % str(self.expected_score)
...
class AnswerScoreBadge(NodeScoreBadge):
    ...
    @property
    def description(self):
        return _('Promise update voted up %s times') % str(self.expected_score)
...
class FavoriteQuestionBadge(AbstractBadge):
    ...
    @property
    def description(self):
        return _('Promise favorited by %s users') % str(self.expected_count)
...
class Scholar(FirstActionBadge):
    ...
    name = _("Scholar")
    description = _('First accepted promise update on your own promise')
...
class SelfLearner(AbstractBadge):
    listen_to = (VoteUpAction, )
    name = _("Self Learner")
    description = _('Updated your own promise with at least %s up votes') % settings.SELF_LEARNER_UP_VOTES
...
class Student(AbstractBadge):
    ...
    listen_to = (VoteUpAction,)
    name = _("Student")
    description = _('Asked first promise with at least one up vote')
...
class Teacher(AbstractBadge):
    ...
    listen_to = (VoteUpAction,)
    name = _("Teacher")
    description = _('Updated first promise with at least one up vote')
...
class Enlightened(AbstractBadge):
    ...
    name = _("Enlightened")
    description = _('First update was accepted with at least %s up votes') % settings.ENLIGHTENED_UP_VOTES
...
class Guru(AbstractBadge):
    ...
    listen_to = (VoteUpAction, AcceptAnswerAction)
    name = _("Guru")
    description = _('Accepted update and voted up %s times') % settings.GURU_UP_VOTES
...
class Necromancer(AbstractBadge):
    ...
    listen_to = (VoteUpAction,)
    name = _("Necromancer")
    description = _('Updated a promise more than %(dif_days)s days later with at least %(up_votes)s votes') % \
...
class Taxonomist(AbstractBadge):
    ...
    listen_to = tuple()
    name = _("Taxonomist")
    description = _('Created a subject used by %s questions') % settings.TAXONOMIST_USE_COUNT
