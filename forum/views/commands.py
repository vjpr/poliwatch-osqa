...
def accept_answer(request, id):
    ...
    if answer.nis.accepted:
        ...
        commands['unmark_accepted'] = [answer.id,question.currentStatusId]
        
        # Update question status
        question.status = question.currentStatusId
        question.save()
        ...
        commands['mark_accepted'] = [answer.id,question.currentStatusId]
        # return new status
        #commands['status'] = [question.currentStatusId]
...
def matching_makers(request):
    if len(request.GET['q']) == 0:
        raise CommandException(_("Invalid request"))

    possible_makers = Politician.objects.filter(name__icontains = request.GET['q']).select_related(depth=1)
    output = ''

    for maker in possible_makers:
        output += ("%s|%s|%s|%s|%s\n" % (maker.id, maker.name, maker.party.name, maker.constituency.name, maker.constituency.region))
